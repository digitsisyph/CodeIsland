...
def _convert_payload(payload: object) -> dict[str, Any]:
    ...
    additional = []
    for limit in payload.get("additional_rate_limits") or []:
        if not isinstance(limit, dict) or not isinstance(limit.get("rate_limit"), dict):
            continue
        name = limit.get("limit_name") or limit.get("metered_feature") or "Model"
        for field, fallback in (("primary_window", "five_hour"), ("secondary_window", "weekly")):
            raw = limit["rate_limit"].get(field)
            window = _window(raw)
            if window is not None:
                period = _window_key(raw, fallback)
                window["name"] = f"{name} {'5h' if period == 'five_hour' else 'weekly'}"
                additional.append(window)
    if additional:
        usage["additional"] = additional
